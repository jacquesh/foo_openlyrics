...
public:
    ...
    PreferencesSearchSources(preferences_page_callback::ptr callback)
        : m_callback(callback)
    {
    }
    ...
    enum
    {
        IDD = IDD_PREFERENCES_SEARCHSOURCES
    };
...
void PreferencesSearchSources::OnMoveUp(UINT, int, CWindow)
{
    ...
    OnActiveSourceSelect(0, 0, {}); // Update the button enabled state (e.g if we moved an item to the bottom we should
...
void PreferencesSearchSources::apply()
{
    ...
    if(musixmatch_enabled && !has_musixmatch_token)
    {
        ...
        query.msg = "You have enabled the 'Musixmatch' source for the OpenLyrics component, but have not provided a "
                    "token. Without a token the Musixmatch source will not work.\r\n\r\nYou can click on the '?' "
                    "button for more information on how to get a token.";
...
void PreferencesSearchSources::SourceListResetFromSaved()
{
    ...
    for(size_t active_source_index = 0; active_source_index < active_source_count; active_source_index++)
    {
        ...
        LRESULT new_index = SendDlgItemMessage(IDC_ACTIVE_SOURCE_LIST,
                                               LB_ADDSTRING,
                                               0,
                                               (LPARAM)src->friendly_name().data());
    ...
    for(size_t entry_index = 0; entry_index < total_source_count; entry_index++)
    {
        ...
        LRESULT new_index = SendDlgItemMessage(IDC_INACTIVE_SOURCE_LIST,
                                               LB_ADDSTRING,
                                               0,
                                               (LPARAM)src->friendly_name().data());
        LRESULT set_result = SendDlgItemMessage(IDC_INACTIVE_SOURCE_LIST,
                                                LB_SETITEMDATA,
                                                new_index,
                                                (LPARAM)&src->id());
        assert(new_index != LB_ERR);
