...
static cfg_auto_combo<UploadStrategy, 2> cfg_upload_strategy(GUID_CFG_UPLOAD_STRATEGY, IDC_UPLOAD_STRATEGY, UploadStrategy::Never, upload_strategy_options);
...
{
public:
    PreferencesUploading(preferences_page_callback::ptr callback) : auto_preferences_page_instance(callback, g_uploading_auto_properties) {}

    // Dialog resource ID - Required by WTL/Create()
    enum {IDD = IDD_PREFERENCES_UPLOADING};
...
{
public:
    const char* get_name() final { return "Uploading"; }
    GUID get_guid() final { return GUID_PREFERENCES_PAGE_UPLOAD; }
