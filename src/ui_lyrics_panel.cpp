...
static int _WrapSimpleLyricsLineToRect(HDC dc, CRect clip_rect, std::tstring_view line, const CPoint* origin)
{
    ...
    {
        size_t leading_spaces = find_first_space(text_outstanding, false);
        text_outstanding.remove_prefix(std::min(leading_spaces, text_outstanding.size()));

        size_t last_not_space = find_last_space(text_outstanding, false);
        ...
        while(true)
        {
            ...
            else
            {
                ...
                const int previous_space_index = int(find_last_space(text_outstanding, true, chars_to_draw - 1));
