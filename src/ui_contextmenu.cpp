...
static contextmenu_group_popup_factory g_ctx_item_factory(GUID_OPENLYRICS_CTX_POPUP,
...
public:
    ...
    {
        return GUID_OPENLYRICS_CTX_SUBGROUP;
    }
    ...
    bool context_get_display(unsigned int index,
                             metadb_handle_list_cref data,
                             pfc::string_base& out_name,
                             unsigned int& out_display_flags,
        ...
        switch(index)
        {
            ...
            case cmd_mark_instrumental:
            {
                ...
                if(track_count == 1)
                {
                    ...
                    msg += "and mark the track as instrumental. OpenLyrics will no longer search for lyrics for this "
                           "track automatically so it will not show any lyrics for this track until you explicitly "
                           "request a search for it.\n\nAre you sure you want to proceed?";
                }
                else
                {
                    msg = "This will delete the lyrics stored locally for the ";
                    msg += std::to_string(track_count);
                    msg += " selected tracks and mark those tracks as instrumental. OpenLyrics will no longer search "
                           "for lyrics for those tracks automatically so it will not show any lyrics for those tracks "
                           "until you explicitly request a search for it.\n\nAre you sure you want to proceed?";
                }
                ...
                const auto async_instrumental =
                    [data_copy, track_count](threaded_process_status& /*status*/, abort_callback& abort)
                {
                    ...
                    {
                        const auto fill_track_info = [&all_track_info](size_t idx, const metadb_v2_rec_t& rec)
                        { all_track_info[idx] = rec; };
                    ...
                    for(size_t i = 0; i < track_count; i++)
                    {
                        ...
                        if(success && handle.has_result())
                        {
                            ...
                            assert(!lyrics.IsEmpty()); // Search should never return empty lyrics. Instead it should
                    ...
                    fb2k::inMainThread2(
                        [failure_count, track_count]()
                        {
                            ...
                            result_query.icon = (failure_count == 0) ? popup_message_v3::iconInformation
    ...
    bool get_item_description(unsigned int index, pfc::string_base& out) override
    {
        switch(index)
        {
            case cmd_show_lyrics: out = "Show saved lyrics (if any) for this track"; return true;
            case cmd_bulksearch_lyrics:
            ...
            case cmd_manualsearch_lyrics:
                ...
                return true;
            case cmd_edit_lyrics: out = "Open the lyric editor with the lyrics for this track"; return true;
            case cmd_mark_instrumental:
